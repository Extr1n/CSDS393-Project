...
# URL of the webpage
url = "https://bulletin.case.edu/arts-sciences/astronomy/astronomy-minor/#programrequirementstext"
...
def get_req(url):
                ...
                if(required):
                    ...
                    if(constraint == "none"):
                        ...
                        if(list[i].get("class")[0] == "orclass"):
                            ...
                            normal = normal = len(list[i].find_all("td"))> 2
                            cl = list[i].find("td").get_text(strip=True)
                            if(cl == "Total Credit Hours"):
                                credithours.append(list[i].find_all("td")[1].get_text(strip=True))
                            else:
                                if(normal) : cl = cl[:4] + cl[5:]
                                else:
                                    hours = list[i].find_all("td")[1].get_text(strip=True)
                                    cl = cl + " " + hours
                                necessary.append([cl])
                    else:
                    ...
                    if(constraint == "none"):
                        #ADD TO electives
                        normal = len(list[i].find_all("td"))> 2
                        cl = list[i].find("td").get_text(strip=True)
                        if(normal) : cl = cl[:4] + cl[5:]
                        electives.append(cl)
                    else:
    ...
    t = []
    if title != None:
        t.append(title.get_text(strip=True))
